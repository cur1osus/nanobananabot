...
# описывай сцены ТОЛЬКО про совершеннолетних, лучше на английском.
# Можно вставить ключ {clothing} — он развернётся в список одежды с фото
# (пресеты с ключом работают в режиме редактирования фото).
# Примеры строк:
#   ("Бикини", "wearing a tiny bikini on the beach"),
#   ("Снять одежду", "removes {clothing}, fully nude"),
ADULT_PRESETS: Final[tuple[tuple[str, str], ...]] = (
    ("Снять одежду", "без без {clothing}, в стрингах ниточкой очень реалистично"),
    (
        "Красивая попа",
        "без {clothing}, в стрингах ниточкой развернута спиной очень реалистично",
    ),
    ("Hot", "чмокает банан сексуально"),
)
