...
from bot.keyboards.music import ik_music_back_home, ik_no_credits
...
def build_lyrics_client() -> AgentPlatformClient:
...
async def ask_for_title(
    ...
    *,
    ...
    await message.answer(
        text,
        reply_markup=await ik_music_back_home(back_to=back_to),
    )
...
async def start_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("Генерация музыки недоступна: %s", err)
        await message.answer(
            "Генерация музыки временно недоступна. Не настроен SUNO_API_KEY."
        )
        await state.clear()
        return
    ...
    try:
        ...
        logger.warning(
            "Не удалось сохранить задачу генерации Suno "
            "task_id=%s user_id=%s chat_id=%s: %s",
            task_id,
            user.user_id,
            message.chat.id,
            err,
