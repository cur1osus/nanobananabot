...
from bot.keyboards.music import (
    ik_music_back_home,
    ik_music_ai_result,
...
from bot.utils.music_helpers import ask_for_title, build_lyrics_client
...
async def lyrics_ai(query: CallbackQuery, state: FSMContext) -> None:
    ...
    await edit_or_answer(
        query,
        text=prompt_text,
        reply_markup=await ik_music_back_home(back_to=back_target),
    )
...
async def lyrics_ai_edit(query: CallbackQuery, state: FSMContext) -> None:
    ...
    if not data.prompt:
        await edit_or_answer(
            ...
            reply_markup=await ik_music_back_home(back_to=_text_menu_back_target(data)),
        )
        return
    await state.set_state(MusicGenerationState.prompt)
    ...
    await edit_or_answer(
        ...
        reply_markup=await ik_music_back_home(back_to=MusicBackTarget.AI_RESULT),
    )
...
async def prompt_received(
    ...
    if data.prompt_source in {"ai", "ai_edit"}:
        ...
        try:
            lyrics = await build_lyrics_client().generate_song_text(
